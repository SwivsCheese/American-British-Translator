      ...
      if(locale === 'american-to-british'){
        ...
        if(youre){
          youre.forEach((a, b) => {
            ...
            const human = newYour.toLowerCase().replace(/^./, char => char.toUpperCase());
            tame = tame.replace(a, human); // stolen from geeksforgeeks
            tOrF = true;
            console.log(tame);
          });
        }
        ...
        if(youre){
          youre.forEach((a, b) => {
            let human = reversed[a.replace(" ", "").toLowerCase()];
            console.log(human);
            const s2 = human.replace(/^./, char => char.toUpperCase()); // stolen from geeksforgeeks
            tame = tame.replace(a, s2);
