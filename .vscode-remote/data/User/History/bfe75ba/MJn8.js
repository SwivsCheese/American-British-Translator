...
const regex3 = /\b(mrs.|mr.|ms.|mx.|dr.|prof.)/gi;
      ...
      if(locale === 'american-to-british'){
        ...
        if(youre){
          youre.forEach((a, b) => {
            ...
            tame = tame.replace(a, `<span class="highlight">${newYour.replace(/^./, char => char.toUpperCase())}</span>`);
            tOrF = true;
            console.log(tame);
          });
        }
        ...
        if(youre){
          youre.forEach((a, b) => {
            ...
            tame = tame.replace(a, `<span class="highlight">${newYork.toLowerCase().replace(/^./, char => char.toUpperCase())} </span>`);
            tOrF = true;
          });
        }  
